...
// Dummy responses as provided in the prompt
...
/**
 * Mock API calls returning Promises with configurable success/failure rates
 */
export const mockApi = {
  fetchHoldings: (shouldFail = false): Promise<Holding[]> => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (shouldFail) {
          reject(new Error("Failed to fetch holdings data from server."));
        } else {
  ...
  fetchCapitalGains: (shouldFail = false): Promise<CapitalGainsResponse> => {
    return new Promise((resolve, reject) => {
      setTimeout(() => {
        if (shouldFail) {
          reject(new Error("Failed to fetch capital gains summary."));
        } else {
